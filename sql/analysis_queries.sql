...
SELECT COUNT(*) AS total_candidates, SUM(target) AS looking_for_change, ROUND(100.0 * AVG(target), 2) AS change_rate_pct
FROM employee;

-- Q1. Ti le doi viec theo trinh do hoc van
SELECT education_level, COUNT(*) AS n, ROUND(100.0 * AVG(target), 2) AS change_rate_pct
FROM employee
...
SELECT relevent_experience, COUNT(*) AS n, ROUND(100.0 * AVG(target), 2) AS change_rate_pct
FROM employee
...
SELECT
    CASE
        ...
        ELSE '20+ yrs'
    END AS experience_band, COUNT(*) AS n, ROUND(100.0 * AVG(target), 2) AS change_rate_pct
FROM employee
...
SELECT
    CASE
        ...
        ELSE 'Very low (<0.70)'
    END AS city_dev_band, COUNT(*) AS n, ROUND(AVG(city_development_index), 3) AS avg_cdi, ROUND(100.0 * AVG(target), 2)  AS change_rate_pct
FROM employee
GROUP BY city_dev_band
ORDER BY change_rate_pct DESC;

-- Q5. Top 10 thanh pho co ti le doi viec cao nhat (lay cac tp >= 100 nguoi cho dang tin)
SELECT city, COUNT(*) AS n, ROUND(AVG(city_development_index), 3) AS cdi, ROUND(100.0 * AVG(target), 2)  AS change_rate_pct
FROM employee
...
SELECT COALESCE(company_type, 'Unknown') AS company_type, COUNT(*) AS n, ROUND(100.0 * AVG(target), 2)     AS change_rate_pct
FROM employee
...
SELECT COALESCE(company_size, 'Unknown') AS company_size, COUNT(*) AS n, ROUND(100.0 * AVG(target), 2) AS change_rate_pct
FROM employee
...
SELECT COALESCE(enrolled_university, 'Unknown') AS enrolled_university, COUNT(*) AS n, ROUND(100.0 * AVG(target), 2) AS change_rate_pct
FROM employee
...
SELECT
    CASE
        ...
        ELSE '4+ yrs'
    END AS tenure_band, COUNT(*) AS n, ROUND(100.0 * AVG(target), 2)  AS change_rate_pct
FROM employee
...
SELECT education_level, relevent_experience, COUNT(*) AS n, ROUND(100.0 * AVG(target), 2)  AS change_rate_pct
FROM employee
WHERE education_level IS NOT NULL
...
SELECT
    CASE
        ...
        ELSE '100+ h'
    END AS training_band, COUNT(*) AS n, ROUND(AVG(training_hours), 1) AS avg_hours, ROUND(100.0 * AVG(target), 2) AS change_rate_pct
FROM employee
